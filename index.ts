...
import { Supertype, supertypeClass, property, remote, Remoteable, Persistable, Bindable, ContainsPersistable } from 'amorphic';
import 'es6-promise';
...
class _Persistent extends Persistable(Supertype) { };  // classes that have no business in any browser

type Constructable<BC> = new (...args: any[]) => BC;
export function AuthenticatedPrincipalMixin<BC extends Constructable<_Persistent>>(Base: BC) {

    @supertypeClass
    class AuthenticatedPrincipalMixin extends Base {
        ...
        @property({ toServer: false, persist: false, getType: () => SecurityContext })
        ...
        changeEmail(email) {
            if (this.securityContext.role == defaultAdminRole.call(this) && (this.role != defaultAdminRole.call(this)))
                return this.amorphicClass.getFromPersistWithQuery(queryFilter.call(this, { email: email })).then(principals => {
    ...
    return AuthenticatedPrincipalMixin;
}
class Principal extends AuthenticatedPrincipalMixin(_Persistent) { }
...
export class SecurityContext extends Remoteable(Persistable(Supertype)) {
    ...
    constructor(principal: AuthenticatedPrincipal, role) {
        super();
...
export abstract class AuthenticatingController extends Bindable(Remoteable(Persistable(Supertype))) {
    ...
    abstract newPrincipal(): AuthenticatedPrincipal;
    ...
    publicLoginFatArrow(page?, forceChange?) {
        ...
        var query = this.amorphicClass.getFromPersistWithQuery(
    ...
    async publicLogin(page?, forceChange?) {
        ...
        var query = this.getPrincipal().amorphicClass.getFromPersistWithQuery(
            queryFilter.call(this,
                {
                    email: {
                        $regex: new RegExp("^" + this.email.toLowerCase().replace(/([^0-9a-zA-Z])/g, "\\$1") + '$'),
                        $options: 'i'
                    }
                }), null, null, null, true);
    ...
    publicChangePasswordFromToken(page) {
        ...
        return this.getPrincipal().amorphicClass.getFromPersistWithQuery(queryFilter.call(this, { email: this.email }), null, null, null, true).then(function(principals) {
            if (principals.length < 1)
                throw {
                    code: "ivalid_password_change_token",
